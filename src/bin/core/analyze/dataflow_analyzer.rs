...
impl CfgAnalyzer {
    ...
    pub fn visit_operand(&mut self, operand: &MirOperand, location: Location) {
        let place = if let MirOperand::Move { place, .. } | MirOperand::Copy { place, .. } = operand
        {
            place
        } else {
            return;
                    ...
                    )))
        {
            match operand {
                ...
                MirOperand::Copy { .. } => {
                    state.insert(LocalStateVariant::Used);
                }
                _ => {}
            }
        }
    ...
    pub fn walk_cfg(
        ...
        locals: &BTreeMap<LocalId, MirType>,
...
use rustc_mir_dataflow::{
    Analysis, MaybeReachable, ResultsVisitor,
    impls::{MaybeInitializedPlaces, MaybeUninitializedPlaces},
    move_paths::MoveData,
    visit_reachable_results,
};
pub fn definitely_lives<'tcx>(
    body: &Body<'tcx>,
    tcx: TyCtxt<'tcx>,
    location_ranges: &LocationRanges,
) -> HashMap<LocalId, Vec<Range>> {
    let move_data = MoveData::gather_moves(body.as_rustc(), *tcx.as_rustc(), |_| true);
    let maybe_uninitialized =
        MaybeUninitializedPlaces::new(*tcx.as_rustc(), body.as_rustc(), &move_data);
    let computed = maybe_uninitialized.iterate_to_fixpoint(
        *tcx.as_rustc(),
        body.as_rustc(),
        Some("definitely_lives"),
    );
    let mut translator = ResultsTranslator::new(&location_ranges, &move_data);
    visit_reachable_results(body.as_rustc(), &computed, &mut translator);
    translator.translated
}
pub fn maybe_lives<'tcx>(
    body: &Body<'tcx>,
    tcx: TyCtxt<'tcx>,
    location_ranges: &LocationRanges,
) -> HashMap<LocalId, Vec<Range>> {
    ...
    let maybe_initialized =
        MaybeInitializedPlaces::new(*tcx.as_rustc(), body.as_rustc(), &move_data);
    let computed = maybe_initialized.iterate_to_fixpoint(
        *tcx.as_rustc(),
        body.as_rustc(),
        Some("definitely_lives"),
    );
...
struct ResultsTranslator<'a, 'tcx> {
    translated: HashMap<LocalId, Vec<Range>>,
    location_ranges: &'a LocationRanges,
    move_data: &'a MoveData<'tcx>,
}
impl<'a, 'tcx> ResultsTranslator<'a, 'tcx> {
    fn new(location_ranges: &'a LocationRanges, move_data: &'a MoveData<'tcx>) -> Self {
        Self {
    ...
    fn push_move_path(
        &mut self,
        state: &<MaybeUninitializedPlaces<'a, 'tcx> as Analysis<'tcx>>::Domain,
        ...
        for idx in state.iter() {
            if let Some(local) = self
                ...
                .move_paths
                .get(idx)
                .map(|v| v.place.as_local())
                .flatten()
                && let Some(range) = self.location_ranges.get(&Location::from_rustc(location))
            {
                self.translated
                    .entry(LocalId::from_rustc(local))
...
impl<'a, 'tcx> ResultsVisitor<'tcx, MaybeUninitializedPlaces<'a, 'tcx>>
    for ResultsTranslator<'a, 'tcx>
{
    fn visit_after_primary_statement_effect(
        &mut self,
        _analysis: &MaybeUninitializedPlaces<'_, 'tcx>,
        state: &<MaybeUninitializedPlaces<'_, 'tcx> as Analysis<'tcx>>::Domain,
        _statement: &rustc_middle::mir::Statement<'tcx>,
        location: rustc_middle::mir::Location,
    ) {
        let mut rev = MixedBitSet::new_empty(state.domain_size());
        rev.insert_all();
        rev.subtract(state);
        self.push_move_path(&rev, location);
    }
...
impl<'a, 'tcx> ResultsVisitor<'tcx, MaybeInitializedPlaces<'a, 'tcx>>
    for ResultsTranslator<'a, 'tcx>
{
    fn visit_after_primary_statement_effect(
        &mut self,
        _analysis: &MaybeInitializedPlaces<'_, 'tcx>,
        state: &<MaybeInitializedPlaces<'_, 'tcx> as Analysis<'tcx>>::Domain,
        _statement: &rustc_middle::mir::Statement<'tcx>,
        location: rustc_middle::mir::Location,
    ) {
        if let MaybeReachable::Reachable(state) = state {
            let mut rev = MixedBitSet::new_empty(state.domain_size());
            rev.insert_all();
            rev.subtract(state);
            self.push_move_path(&rev, location);
